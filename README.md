...
1. Add a space with the `+` button in the top tab bar; right-click to rename or delete it.
2. Inside each space, use `+` to create a board, then `+` inside a board to add cards (link/note/todo).
3. Drag tabs from the left drawer to save them as cards, or click a card's link to open it in a new tab.
4. Use the top-right search box to quickly filter cards, and the star tab to see only favorites.
5. The board header's `x sites` button opens all link cards in that board as a new tab group.
...
| [docs/](docs/) | Detailed architecture, guides, and API reference |
